...
if ((function_exists('is_multisite') && is_multisite()) || ($single_site = true)) {
    ...
    function indreams_image_resize($img_url, $width, $height, $crop = true, $quality = 100) {
        ...
        if (isset($file_path['host']) && $_SERVER['HTTP_HOST'] != $file_path['host'] && $file_path['host'] != '') {  // The image is not locally hosted
            ...
            if ($findit === false) {
                $file_path = indreams_document_root($file_path['path']) . $file_path['path'];
            } else {
                $expath = $file_path['path'];
 ...
 * @return type 
 */
function indreams_thumbnail_resize($attach_id, $img_url, $width, $height, $crop = false, $jpeg_quality = 90) {
    ...
    if ($image_src[1] > $width || $image_src[2] > $height) {
        ...
        if (isset($cropped_img_path) && file_exists($cropped_img_path)) {
        ...
        if ($crop == false) {
            ...
            $resized_img_path = isset($no_ext_path) ? $no_ext_path . '-' . $proportional_size[0] . 'x' . $proportional_size[1] . $extension : '';
            // checking if the file already exists
            if (isset($resized_img_path) && file_exists($resized_img_path)) {
